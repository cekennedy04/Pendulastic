"""
run_detrpose_participant_1.py
==============================
Runs DETRPose-S on Participant 1 trial videos and outputs knee-angle CSVs
matching the project naming scheme.

Model: DETRPose-S (11.5M params, 67.0 AP COCO val2017)
Weights: models/DETRPose-main/weights/detrpose_hgnetv2_s.pth
Config:  models/DETRPose-main/configs/detrpose/detrpose_hgnetv2_s.py

DETRPose returns:
  scores     : (N,)      — per-person confidence
  labels     : (N,)      — class labels
  keypoints  : (N, 17, 2) — absolute pixel (x, y), COCO skeleton, no per-kpt conf

COCO keypoint indices (left/right):
  left:  hip=11, knee=13, ankle=15
  right: hip=12, knee=14, ankle=16

Person-level score is used as the per-keypoint score (best proxy available).

Run from the Pendulastic directory:
    .venv\\Scripts\\python.exe run_detrpose_participant_1.py
"""
...
# Add DETRPose source root to path
...
def _pick_leg_detr(kp17, person_score):
    """
    kp17: (17, 2) absolute pixel coords.
    Returns (leg_label, hip_xy, knee_xy, ankle_xy, score).
    Since DETRPose gives no per-keypoint confidence, use the person score for all.
    Picks the side with the larger vertical ankle-knee-hip span (the tested leg
    hangs lower, which gives a larger pixel-y range on screen).
    """
    def span(h, k, a):
        ys = [h[1], k[1], a[1]]
...
def process_trial(trial_num, model, device):
    video_path = os.path.join(VIDEO_DIR, f"Trial_{trial_num}.avi")
    if not os.path.isfile(video_path):
        print(f"[SKIP] {video_path} not found"); return

    out_name = f"P_1_Pos_2_H_Joint-Level_T_{trial_num}_detrpose_s_{SCORE_THRESH}.csv"
    ...
    with torch.no_grad():
        while True:
            ...
            scores_np = scores_t[0].cpu().numpy()   # (N,)
            kpts_np   = kpts_t[0].cpu().numpy()     # (N, 17, 2)

            # Pick the highest-scoring person above threshold
            valid = np.where(scores_np >= SCORE_THRESH)[0]
            if len(valid) == 0:
                rows.append(_nan_row(frame_idx, t_sec))
            else:
                best   = valid[int(np.argmax(scores_np[valid]))]
                kp17   = kpts_np[best]           # (17, 2)
    ...
    print(f"  Done: {len(rows)} frames, {valid_n} with valid angle -> {out_path}")
...
def main():
    ...
    for trial in TRIALS:
        process_trial(trial, model, device)
