...
**Live deployment:** https://pendulastic-app.vercel.app — open it in Safari on
the phone and use *Share → Add to Home Screen*. Install it; do not just
bookmark it (see [Deploying to Vercel](#deploying-to-vercel-the-live-app)).
...
## Deploying to Vercel (the live app)

The app is deployed to Vercel project `pendulastic-app`, live at
**https://pendulastic-app.vercel.app**. Deploying is a **local build followed
by a folder upload** — it is *not* triggered by pushing to GitHub, and cannot
be (see the next section for why). To ship a new build:

```
cd webapp
npm install                                   # first time only
npm run build:dist                            # rebuilds the wasm, assembles webapp/dist/
npx vercel login                              # first time only; opens the browser
npx vercel deploy --prod --yes --project pendulastic-app dist
```

Prerequisites are the same as the build step above: Node, `cargo` with the
`wasm32-unknown-unknown` target, and a matching `wasm-bindgen` CLI. `npx vercel`
downloads the Vercel CLI on first use; you need to be a member of the Vercel
account that owns `pendulastic-app` (or create your own project — the CLI
prompts to create one if the name does not exist under your account).

Things that will bite you:

- **Do not link the GitHub repo in the Vercel dashboard.** Vercel infers a
  *Python* framework preset from the `.py`-heavy repo root, stores it
  server-side, and applies it before reading the uploaded `vercel.json`, so
  every deploy fails with "No python entrypoint found" and re-uploading a
  corrected config does not help. Always deploy by CLI upload into a project
  that was created by the CLI.
- **`build:dist` writes `dist/vercel.json` for you** (alongside `_headers`):
  `framework: null`, empty build/install commands, and `Cache-Control:
  no-cache` on `sw.js` and `src/build-id.js`. Vercel ignores `_headers`, so
  without `vercel.json` installed phones never notice a new build exists.
  `tests/dist-build.test.js` keeps the two files in step; do not hand-edit
  either.
- **A rebuild requires a redeploy.** Nothing keeps the live site in sync with
  the repo. After any change under `mobile-imu-core/src/` or `webapp/src/`,
  run the two commands above again.
- **Testers must *install* the app, not bookmark it.** iOS evicts IndexedDB
  for ordinary sites after ~7 days of disuse, and recordings live on the
  phone. A Home Screen install is exempt. Export trials (in-app export) before
  clearing site data or changing hosts.

To verify a deploy landed: open the URL in a private Safari tab and check the
build id shown at the bottom of the app's Session view matches `dist/src/build-id.js`.
