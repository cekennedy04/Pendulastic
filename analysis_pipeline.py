...
MOTIVE_BATCH_PROCESSOR = (
    r"C:\Program Files\OptiTrack\Motive\MotiveBatchProcessor.exe"
)
# Maximum seconds to wait for MotiveBatchProcessor to finish.  Large .tak
# collections may need this raised.
MOTIVE_BATCH_TIMEOUT = 300
...
# =============================================================================
# 10e-b. OptiTrack .tak → .csv batch export  (MotiveBatchProcessor)
# =============================================================================
def _export_optitrack_takes(optitrack_root=None):
    """
    Scan optitrack_root for .tak files that have no matching .csv, generate a
    temporary NMotive C# script, and run MotiveBatchProcessor.exe to export each
    take as a rigid-body quaternion CSV.

    The generated script calls:
        Application.Open(takPath)
        take.SolveRigidBodies()
        take.ExportTrackingData(csvPath, exporter)   # ExporterCSV, Quaternion, no markers
        Application.Close()

    API note: method/enum names target Motive 3.x.  If your version differs, look
    at the script path printed below and adjust the template inside this function
    (search for "NMotive Batch Export Script" to find it).

    Skips silently when:
      - MOTIVE_BATCH_PROCESSOR binary does not exist.
      - No .tak files are found (or all already have .csv counterparts).

    Returns:
        int — number of .tak files targeted for export.
    """
    import tempfile as _tmp
    import subprocess as _sp

    root = optitrack_root or OPTITRACK_DATA_ROOT
    ...
    if not os.path.isfile(MOTIVE_BATCH_PROCESSOR):
        print(
            ...
            f"    (b) Export CSVs manually in Motive: File > Export > CSV\n"
            f"        (Rotation ON, Format Quaternion, Markers OFF)."
        )
        return 0

    # Collect .tak files that do not yet have a matching .csv.
    pending = []
    for dirpath, _, filenames in os.walk(root):
        for fname in sorted(filenames):
            if fname.lower().endswith(".tak"):
                tak = os.path.join(dirpath, fname)
                csv = os.path.splitext(tak)[0] + ".csv"
    ...
    # -------------------------------------------------------------------------
    # NMotive Batch Export Script — template
    # Motive 3.x API reference: Motive > Help > NMotive API Reference
    # -------------------------------------------------------------------------
    script_body = f"""\
// ============================================================================
// NMotive batch export script — generated by analysis_pipeline.py
// ============================================================================
// Targets: OptiTrack Motive 3.x  (NMotive API)
// If a class or method name does not compile, open Motive > Help > NMotive API
// Reference and verify:  ExporterCSV, RotationFormat enum, Application.Open,
// Take.SolveRigidBodies, Take.ExportTrackingData.
//
// Export settings:
//   Rotation data:   ON
//   Rotation format: Quaternion (global)
//   Marker data:     OFF
// ============================================================================
...
class PendulasticExport
{{
    // .tak files injected at pipeline run-time; script skips any that already
    // have a matching .csv so re-runs are safe.
    static readonly string[] TAK_PATHS = {{
{cs_path_lines}
    }};

    static void Main()
    {{
        foreach (string takPath in TAK_PATHS)
        {{
            if (!File.Exists(takPath))
            {{
                Console.WriteLine("[skip] Not found: " + takPath);
                continue;
            }}

            string csvPath = Path.ChangeExtension(takPath, ".csv");
            if (File.Exists(csvPath))
            {{
                Console.WriteLine("[skip] CSV exists: " + Path.GetFileName(csvPath));
                continue;
            }}

            Console.WriteLine("[open] " + takPath);
            Application.Open(takPath);

            Take take = Application.CurrentTake;
            if (take == null)
            {{
                Console.WriteLine("[error] Take did not load: " + Path.GetFileName(takPath));
                Application.Close();
                continue;
            }}

            // Ensure rigid-body tracking is solved before exporting.
            take.SolveRigidBodies();

            ExporterCSV exporter = new ExporterCSV();
            exporter.Rotation       = true;
            exporter.RotationFormat = RotationFormat.Quaternion;
            exporter.Markers        = false;

            Console.WriteLine("[export] -> " + csvPath);
            take.ExportTrackingData(csvPath, exporter);

            Application.Close();
            Console.WriteLine("[done]   " + Path.GetFileName(csvPath));
        }}

        Console.WriteLine("\\nPendulastic export complete.");
    }}
}}
"""

    # Write the script next to the data root so the working directory is sane.
    tmp_fd, tmp_cs = _tmp.mkstemp(
        suffix=".cs", prefix="pendulastic_export_", dir=root
    )
    os.close(tmp_fd)
...
def run_full_pipeline(root_dir, render_videos=True, download=True, grid=None):
    ...
    Single-command, hands-free execution chain:
      ...
      Step 1 — OptiTrack .tak → CSV export
        Scan OptiTrack_Tracking_Data/ for binary .tak recording files that have
        no matching .csv.  If MotiveBatchProcessor.exe is present, generate a
        NMotive C# script on the fly and run it to export rigid-body quaternion
        CSVs automatically.  Skips silently when the binary is absent or all
        .tak files already have .csv counterparts.

      Step 2 — Weight resolution
      ...
      Step 3 — Model × complexity × threshold grid sweep
    ...
    # ── Step 2: weight resolution ─────────────────────────────────────────────
    if download:
        print("\n[2/5] Verifying / downloading model weights ...")
        print("=" * 70)
        ensure_local_models()
    ...
    # ── Steps 4 & 5: evaluate + leaderboard ──────────────────────────────────
    print("\n[4/5] Evaluating angular knee RMSE vs OptiTrack gold standard ...")
    print("=" * 70)
    evaluate_results_json(json_path)

    print("\n[5/5] Full pipeline complete.")
    return full_results
